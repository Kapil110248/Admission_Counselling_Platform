...
import { GraduationCap, Lock, Mail, Users, ShieldAlert, UserCheck, ArrowRight, Sparkles } from 'lucide-react';
...
export default function Login() {
  ...
  const handleRoleLogin = () => {
    ...
    else if (email.includes('counsellor')) navigate('/counsellor-dashboard');
    else if (email) navigate('/student-dashboard'); // If something is filled but no specific role, default to student
  };
      ...
      <div className="w-full max-w-5xl bg-white rounded-[2rem] shadow-2xl overflow-hidden flex flex-col md:flex-row z-10 border border-white/50 relative">
        ...
        >
          ...
          <motion.div variants={itemVariants}>
            <h1 className="text-2xl md:text-3xl font-bold text-slate-800 tracking-tight">Welcome Back 👋</h1>
            <p className="text-slate-500 text-sm mt-2">Please log in to your account to continue.</p>
          </motion.div>
          ...
          <div className="space-y-4 mt-8">
            ...
            <motion.div variants={itemVariants}>
               <div className="flex justify-between items-center mb-1.5 ml-1 mr-1">
                 ...
                 <button className="text-xs font-bold text-primary-600 hover:text-primary-700 transition-colors">Forgot?</button>
               </div>
               <div className="relative group">
                  <input 
                    type="password" 
          ...
          <motion.p variants={itemVariants} className="text-center mt-10 text-xs text-slate-500 font-medium">
             Don't have an account? <button className="text-primary-600 font-bold hover:underline">Register now</button>
          </motion.p>
        </motion.div>
      </div>
    </div>
